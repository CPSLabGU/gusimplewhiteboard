...
/** WB Ptr Class: VisionFieldFeatures @brief Nil */ 
			class VisionFieldFeatures_t VisionFieldFeatures_msg(wbd);
			VisionFieldFeatures v = VisionFieldFeatures_msg.get(); 
			v.from_string(message_content);
			VisionFieldFeatures_msg.post(v);
			return true;
		}
#else
			return false;
...
/** WB Ptr Class: WhistleBlown @brief Nil */ 
			...
			WhistleBlown_msg.post(atoi(message_content.c_str()));
			return true;
		}
...
/** WB Ptr Class: VolumeControl @brief Nil */ 
			...
			uint8_t v = VolumeControl_msg.get(); 
			v.from_string(message_content);
			VolumeControl_msg.post(v);
			return true;
		}
#else
			return false;
